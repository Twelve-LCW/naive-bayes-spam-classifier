...
├── models/
│   ├── __init__.py
│   ├── multinomial_nb.py                 # B同学：Multinomial Naive Bayes 实现
│   └── bernoulli_nb.py                   # A同学：Bernoulli Naive Bayes 实现
│
├── utils/
│   ├── data_loader.py                    # （建议新增）统一加载 cleaned data + indices + vocab
│   └── model_io.py                       # 模型序列化保存/加载工具（如 pickle）
│
├── train/
│   ├──train_multinomial.py                  # B同学训练脚本 → 输出 multinomial_model.pkl
│   ├──train_bernoulli.py                    # A同学训练脚本 → 输出 bernoulli_model.pkl
│
├── evaluate/
├── predict_interface.py                  # 统一预测接口（输入原始文本 → 输出 0/1）
├── evaluate_comparison.py                # 加载两个模型 + 测试集 → 输出 Precision/Recall 对比
│
├── saved_models/                         # 最终训练好的模型文件
